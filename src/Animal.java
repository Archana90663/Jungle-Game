...
public class Animal implements ParkGUI {
  ...
  public Animal(JunglePark processing, String imageFileName) {
    this(processing, randGen.nextInt(processing.width),
  ...
  /**
   * Checks whether one of the animals is close to another animal's neighborhood.
   *
   * @param otherAnimal is the animal which is close to the other animal
   * @param range is the distance with respect to the other animal's neighborhood
   * @return true or false if the range is less than or equal to the distance to the animal's
   *         neighborhood.
   */
  public boolean isClose(Animal otherAnimal, int range) {
    if (range <= (int) distance(otherAnimal)) {
      return false;
    } else {
      return true;
    }
